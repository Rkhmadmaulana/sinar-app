...
<head>
    <meta charset="utf-8">
    <title>Laporan Rujukan Masuk</title>
    <style>
        body {
...
<body>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width: 8%;">No. Rujuk</th>
                <th style="width: 10%;">No. Rawat</th>
                <th style="width: 6%;">No. RM</th>
                <th style="width: 12%;">Nama Pasien</th>
                <th style="width: 5%;">Umur</th>
                <th style="width: 12%;">Perujuk</th>
                <th style="width: 10%;">Alamat Perujuk</th>
                <th style="width: 8%;">Dokter Perujuk</th>
                <th style="width: 10%;">Diagnosa Awal</th>
                <th style="width: 8%;">Poli Rujukan</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td class="text-left">{{ $item->no_rujuk }}</td>
                <td class="text-left">{{ $item->no_rawat }}</td>
                <td>{{ $item->no_rkm_medis }}</td>
                <td class="text-left">{{ $item->nm_pasien }}</td>
                <td>{{ $item->umur }}</td>
                <td class="text-left">{{ $item->perujuk }}</td>
                <td class="text-left">{{ $item->alamat_perujuk }}</td>
                <td class="text-left">{{ $item->dokter_perujuk }}</td>
                <td class="text-left">{{ $item->kd_penyakit }} - {{ $item->nm_penyakit }}</td>
                <td class="text-left">{{ $item->nm_poli }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="page-break">
        <div class="statistics-title">
            RINGKASAN STATISTIK RUJUKAN MASUK
        </div>
        ...
        <div style="text-align: center; margin: 20px 0; padding: 10px; background-color: #e9ecef; border: 1px solid #000;">
            <strong>Total Pasien Rujukan Masuk: {{ $totalPasien }}</strong>
        </div>
        
        <!-- Statistics by Date -->
        <div class="stat-header">1. JUMLAH PASIEN BERDASARKAN TANGGAL REGISTRASI</div>
        ...
        <div class="stat-header">2. JUMLAH PASIEN BERDASARKAN PERUJUK</div>
        <table class="stat-table">
            <thead>
                <tr>
                    <th style="width: 40%;">Perujuk</th>
                    <th style="width: 20%;">Jumlah Pasien</th>
                    <th style="width: 20%;">Persentase</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    <td class="text-left">{{ $perujuk ?: 'Tidak Tercatat' }}</td>
                    <td>{{ $jumlah }}</td>
                    <td>{{ $totalPasien > 0 ? number_format(($jumlah / $totalPasien) * 100, 2) : 0 }}%</td>
                </tr>
                @empty
        ...
        <div class="stat-header">4. JUMLAH PASIEN BERDASARKAN POLI RUJUKAN</div>
        <table class="stat-table">
            <thead>
                <tr>
                    <th style="width: 40%;">Poli</th>
                    <th style="width: 20%;">Jumlah Pasien</th>
                    <th style="width: 20%;">Persentase</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    <td class="text-left">{{ $poli ?: 'Tidak Tercatat' }}</td>
                    <td>{{ $jumlah }}</td>
                    <td>{{ $totalPasien > 0 ? number_format(($jumlah / $totalPasien) * 100, 2) : 0 }}%</td>
                </tr>
                @empty
